...
func InitCacheProxyServer() *server.CacheProxyServer {
	wire.Build(context.Background, conf.GetGlobalConfig, db.NewCacheProxy, cachemanager.NewCacheManager, server.NewHttpProxyServer, server.NewCacheProxyServer)
	return new(server.CacheProxyServer)
}

func InitCacheManager() cachemanager.CacheManager {
	wire.Build(context.Background, conf.GetGlobalConfig, db.NewCacheProxy, cachemanager.NewCacheManager)
	return cachemanager.CacheManager(nil)
}
